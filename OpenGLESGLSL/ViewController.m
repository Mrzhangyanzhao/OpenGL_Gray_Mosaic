...
//     [self setupShaderProgramWithName:@"namal"]; // 一开始选用默认的着色器
    ...
    for (int i=0; i<10; i++) {
        if (i/2*2 == i) {
            NSLog(@"相等:i=%d,结果：%d",i,i/2*2);
        }else{
            NSLog(@"不相等:i=%d,结果：%d",i,i/2*2);
        }
    }
...
- (void)setupFilterBar {
    ...
    NSArray *dataSource = @[@"无",@"灰度",@"颠倒",@"马赛克1",@"三角形",@"六边形"];
    filerBar.itemList = dataSource;
}
- (void)filterBar:(FilterBar *)filterBar didScrollToIndex:(NSUInteger)index{
    NSArray *filterArr = @[@"namal",@"Gray",@"Reversal",@"Mosaic",@"TriangularMosaic",@"HexagonMosaic"];
